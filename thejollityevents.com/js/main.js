...
/* ------------------------------------------------------------------
   Programs nav dropdown — tap-to-toggle for touch/mobile
   (desktop opens on hover via CSS)
   ------------------------------------------------------------------ */
document.querySelectorAll(".nav-item-drop > a").forEach((link) => {
...
document.addEventListener("click", (e) => {
  if (!e.target.closest(".nav-item-drop")) {
    document.querySelectorAll(".nav-item-drop.open").forEach((li) => li.classList.remove("open"));
  }
});
