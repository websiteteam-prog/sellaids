<?php
/**
 * Shared site footer — The Jollity Events
 */
?>
  <!-- ================= Footer ================= -->
  <footer class="site-footer">
    <div class="container">
      <div class="footer-grid">
        <div class="footer-brand">
          <span class="brand-name">The <span>Jollity</span> Events</span>
          <p>Connecting, Engaging, Empowering… Our Seniors! Helping every senior stay active, social, creative, and Jolly.</p>
          <p class="footer-cap">“Inspiring active minds, joyful hearts, and purposeful living!”</p>
        </div>
        <div class="footer-col">
          <h4>Quick Links</h4>
          <ul>
            <li><a href="activities.php">Programs</a></li>
            <li><a href="calendar.php">Calendar</a></li>
            <li><a href="about.php">About Us</a></li>
            <li><a href="contact.php">Contact</a></li>
            <li><a href="testimonials.php">Testimonials</a></li>
          </ul>
        </div>
        <div class="footer-col">
          <h4>Get Started</h4>
          <ul>
            <li><a href="register.php">Register Now</a></li>
            <!--<li><a href="contact.php">Enquire Us</a></li>-->
            <li><a href="our-jollies.php">Our Jollies</a></li>
            <li><a href="happy-moments.php">Happy Moments</a></li>
          </ul>
        </div>
      </div>
      <div class="footer-bottom">
        <span>© <?php echo date('Y'); ?> The Jollity Events. All rights reserved.</span>
        <span>Made with 💛 for our Seniors</span>
      </div>
    </div>
  </footer>

  <script src="js/main.js"></script>
</body>
</html>
