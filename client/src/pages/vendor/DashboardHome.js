...
const DashboardHome = () => {
  ...
  // YE SABSE IMPORTANT CHANGE HAI – REAL-TIME NAME + INITIALS
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const summaryUrl = `${process.env.REACT_APP_API_URL}/api/product/dashboard?vendor_id=${vendor.id}`;
      ...
      const productsUrl = `${process.env.REACT_APP_API_URL}/api/product/products-list?vendor_id=${vendor.id}&page=1&limit=5`;
      const productsRes = await axios.get(productsUrl, { withCredentials: true });

      setRecentProducts(productsRes.data.products || productsRes.data.data?.products || []);
    } catch (err) {
    ...
    <div className="p-4 sm:p-6  min-h-screen">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow overflow-x-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => navigate("/vendor/all-products")} className="text-sm text-blue-600 hover:underline">
            View All
          </button>
        </div>
            ...
            {recentProducts.length > 0 ? (
              recentProducts.map((product, idx) => (
                <tr key={product.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2 font-bold">₹{Number(product.selling_price).toLocaleString()}</td>
                  <td className="p-2">
                    <span className={`px-2 py-1 text-xs rounded ${product.status === "Approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                      {product.status}
                    </span>
                  </td>
                  ...
                  <td className="p-2">
                    <Link to={`/vendor/view-product/${product.id}`} className="text-orange-600 hover:text-orange-800">
                      View
                    </Link>
                  </td>
                </tr>
