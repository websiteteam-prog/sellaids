...
const DashboardHome = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [summaryRes, ordersRes] = await Promise.all([
          ...
          axios.get(`${baseURL}/api/vendor/recent-orders?limit=4`, config),
        ]);
  ...
  const chartData =
    activeTab === "sales" ? summary.salesOverview : summary.ordersOverview;
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow mb-8 overflow-x-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
          ...
          <div className="flex space-x-2">
            <button onClick={() => setActiveTab("sales")} className={`px-3 py-1 rounded text-sm ${activeTab === "sales" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}>Sales</button>
            <button onClick={() => setActiveTab("orders")} className={`px-3 py-1 rounded text-sm ${activeTab === "orders" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}>Orders</button>
          </div>
        </div>
              ...
              {recentOrders.length > 0 ? (
                recentOrders.map((order, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      <span className={`px-2 py-1 text-xs rounded ${colorMap[order.color]?.bg || "bg-gray-100"} ${colorMap[order.color]?.text || "text-gray-700"}`}>
                        {order.status}
                      </span>
                    </td>
